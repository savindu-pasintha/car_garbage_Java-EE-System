...
<script type="text/javascript">
    var selectedItemstvalueinputidArray = [];
	...
	function totalAdd(index,price) {
		var dbl = parseFloat(price);
		totalValue = totalValue + dbl;
		var ht = document.getElementById("totalHTag");
		ht.innerText = totalValue;
		
		//[i] to set id as a value [,,,3,4,,6,7]
		selectedItemstvalueinputidArray[index] = index;
		//console.log(selectedItemstvalueinputidArray.toString());
		
		document.getElementById("tatalvalueinputid").value=totalValue;
		document.getElementById("selectedItemstvalueinputid").value=selectedItemstvalueinputidArray.toString();
		
	}

	function totalWhenRemoveClick(index,pr) {
		...
		//[i] to set null [,,,3,4,,6,7]
		selectedItemstvalueinputidArray[index] = "";
		console.log(selectedItemstvalueinputidArray.toString());
	...
	function addRow(selectedArrayItemsParameter) {
		totalAdd(selectedArrayItems[0],selectedArrayItems[2]);
	...
	function removeRow(oButton) {
		...
		var idNo = event.srcElement.id;
		var getRemoveButtonPriceAttributeValue = document.getElementById(idNo).getAttribute("price");
		
		totalWhenRemoveClick( document.getElementById(idNo).getAttribute("code"),getRemoveButtonPriceAttributeValue);
...
<%
try{
 /*http://localhost:8080/car_garbage/invoice.jsp?
		 name=&
		 card=&
		 month=January&
		 year=2015&
		 ...
		 selectedItems=%2C%2C%2C3%2C4%2C5%2C%2C7%2C8%2C9&
		 total=1200&
		 paybtn=	 
*/
...
String items=request.getParameter("selectedItems");

if(n!= "" && m!="" && c!= "" && nic != "" && t!="" && cd!="" && total!=""
&& n!= null && m!=null && c!= null && nic != null && t!=null && cd!=null && total!=null && items!=null){
	if(new Customer_Operation().addCustomer(nic,cd, n, t)){
		if(new Order_Operation().addOrder(nic,total,items)){
			%>
			<script type="text/javascript">
			  swal("Payment", "Payment Succesfull Completed!", "success");
			  window.location.href="./invoice.jsp?status=succes";
            </script>
		...
		<script type="text/javascript">
			  swal("Payment", "Payment Failed !", "error");
			 // window.location.href="./invoice.jsp?status=succes";
           </script>
		<% 
		   //response.sendRedirect("./index.jsp?status=notSaved");
		}%>
	...
	<div class="card" id="paymentCard">
		...
		<div class="card-body">
			<form action="index.jsp" method="get">
				<div class="one">
					<label for=''>Name on card</label> <input size="50" name="name"
						placeholder='Shamma Mohommed' type='text'>
				</div>

				<div class="two">
					<label for=''>Card Number</label> <input size="16" name="card"
				...
				<div id="total" class="text-center p-2">
					...
					<div class="col-12 p-2">
						...
						<div class="col-12 p-2">
							<input name="telephone" required type="tel" size="12"
								placeholder="Mobile : + 97 156 800 0000"
								class="form-control" />
							<input name="selectedItems" id="selectedItemstvalueinputid"  hidden="true"/>	
							<input name="total"  id="tatalvalueinputid" hidden="true" />	
						</div>
